...
inline bool doubleEqual(double a, double b) { return abs(a - b) < 1E-6; }
...
struct CSCDetID {
  ...
  friend ostream& operator<<(ostream& os, const CSCDetID& id) {
    os << "E:" << id.Endcap << " S:" << id.Station << " R:" << id.Ring
       << " Chamber:" << id.Chamber << " layer:" << id.Layer;
    return os;
  }
};

// GEMDetID for GE11 and GE21, use station=0 and ring=0 for ME0
// GEMDetId from CMSSW: GEMDetId(int region, int ring, int station, int layer,
// int chamber, int ieta) region, -1/+1, ring=1, station=0,1,2, layer=1,2. 0 is
// superchamber, etapartition=0-16 for ME0  ME0DetId(int region, int layer, int
// chamber, int roll); with ch0-18,region=-1/+1 layer=0-6, and roll0-10 ME0:
// https://github.com/cms-sw/cmssw/blob/master/DataFormats/MuonDetId/interface/ME0DetId.h
struct GEMDetID {
  double Endcap;
  double Station;
  double Ring;
...
struct SimHit {  // GEM and CSC?
  ...
  bool operator==(const SimHit& sh) const {
    return sh.WireGroup == this->WireGroup && sh.Strip == this->Strip &&
           sh.PdgId == this->PdgId;
  }
...
struct SIMTRACK {
  ...
  friend ostream& operator<<(ostream& os, const SIMTRACK& st) {
    os << "TrackID:" << st.trackId << " Type:" << st.type
       << " Primary:"  //<< st.isPrimary
       << " Charge:" << st.charge << " Momentum:(" << st.momentumX << ","
       << st.momentumY << "," << st.momentumZ << ")";
    return os;
  }
};
...
struct Strips {
  ...
  bool operator==(const Strips& st) const { return st.Strip == this->Strip; }

  bool operator<(const Strips& st) const { return this->Strip < st.Strip; }
  Strips() {
    this->Strip = -99;
    this->ADCTotal = -99;
    this->MaxADC = -99;
...
struct GEMPad {
  ...
  bool operator<(const GEMPad& gempad) const { return this->Pad < gempad.Pad; }
};
