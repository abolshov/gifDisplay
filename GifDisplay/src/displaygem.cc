...
void GEMPadDisplay(TString address, GEMDetID id, CSCDetID CSCid,
                   vector<GEMSIMHIT>& simhit, vector<GEMPAD>& gempads,
                   vector<GEMCLUSTER>& gemclusters,
                   vector<GEMDetID>& usedChamber, vector<CSCIDLCTs>& alllcts,
                   vector<CSCIDLCTs>& alllcts_emul, int Run, int Event,
                   bool addEmulation, bool doGEMCSC, int doDebug) {
  gStyle->SetPalette(55);
  const int NPad = 192;
  const int NRoll = 8;
  const int NLayer = 2;

  // if (std::find(usedChamber.begin(), usedChamber.end(), id) ==
  // usedChamber.end()) return;
  ...
  if (doDebug > 2)
    cout << "start to display GEM, total in canvas " << npad_canvas << endl;
  ...
  if (doGEMCSC) {
    ...
    TH2F* extrapLCT =
        new TH2F("extrapLCT", "", NPad, 0, NPad, nybins, 0, nybins);
    TH2F* unextrapLCT =
        new TH2F("unextrapLCT", "", NPad, 0, NPad, nybins, 0, nybins);
    TPaveText* LCTLegend = new TPaveText(0.75, .7, 0.9, 0.96, "NDC");

    vector<CorrelatedLCT> lcts;
    if (addEmulation)
      lcts = findStubsInChamber(CSCid, alllcts_emul);
    else
      lcts = findStubsInChamber(CSCid, alllcts);

    // if (lcts.size() == 1 && lcts.at(0).quality >= 5) {
    //     cout << "-------" << "station  "<< CSCid.Station <<" endcap "<<
    //     CSCid.Endcap <<" chamber "<< CSCid.Chamber << "-----" << endl;
    // }
  ...
  TH2F* simhitDis =
      new TH2F("padDis", "", NPad, 0, NPad * 2, nybins, 0, nybins);
  TPaveText* pt2 = new TPaveText(0.4, .92, 0.6, 0.96, "NDC");
  if (doSimHit) {
    if (doDebug > 2)
      cout << "dipslay GEM simHits at ipad canvas " << ipad << endl;
    c1->cd(ipad)->SetGridy();
    ipad++;
    gPad->SetTopMargin(0.1);
    gPad->SetBottomMargin(0.15);
  ...
  // show GEMClusters
  if (doDebug > 2)
    cout << "dipslay GEM Cluster at ipad canvas " << ipad << " superchamber "
         << id << endl;
  c1->cd(ipad)->SetGridy();
  ipad++;
  ...
  // chamber, run, event
  stringstream ss;
  ss << "GEM SuperChamber " << (id.Endcap == 1 ? "+" : "-") << id.Station << "/"
     << id.Ring << "/" << id.Chamber << " ";
  ...
  for (int l = 1; l <= NLayer; l++) {
    for (int ieta = 1; ieta <= NRoll; ieta++) {
      ...
      for (unsigned int i = 0; i < clts.size(); i++) {
        ss_clusters[iclst] << "Cluster #" << iclst << " Layer " << l << " Roll "
                           << ieta << " first Pad " << clts[i].firstPad
                           << " size " << clts[i].size << " BX "
                           << clts[i].BX + 8;
...
void SetSaveNameLegendNameGEM(TString& name, TString& legendName,
                              TString address, GEMDetID id, int Run,
                              int Event) {
  TString space = "                                   ";
  if (id.Endcap == 1) {
    legendName = "GE+" + NumberToString(id.Station) + "/" +
    ...
    name = address + "/" + NumberToString(Run) + "_" + NumberToString(Event) +
           "_GEPlus" + NumberToString(id.Station) + "_" +
  ...
  if (id.Endcap == 2) {
    legendName = "GE-" + NumberToString(id.Station) + "/" +
                 ...
                 "Event #" + NumberToString(Event);
    name = address + "/" + NumberToString(Run) + "_" + NumberToString(Event) +
           "_GEMinus" + NumberToString(id.Station) + "_" +
           NumberToString(id.Ring) + "_" + NumberToString(id.Chamber);
  }
}

void GEMSimHitDisplay(GEMDetID id, vector<GEMDetID>& allIds,
                      vector<GEMSIMHIT>& gemsimHits, TH2F* stripDis,
                      int doDebug) {
  ...
  for (int i = 1; i < 3; i++) {  // in each interesting layer has strip hits
    for (int ieta = 1; ieta <= 8; ieta++) {
      ...
      stripDis->GetYaxis()->ChangeLabel(ybin + 1, -1, -1, -1, -1, -1,
                                        label[ybin].str().c_str());
      if (i == 1)
        stripDis->GetYaxis()->ChangeLabel(ybin + 2, -1, -1, -1, -1, -1, "--");

      if (doDebug > 2)
        cout << "GEMSimhit display id " << tempId << " label "
             << label[ybin].str() << " ybin " << ybin << endl;
      auto it = find(allIds.begin(), allIds.end(), tempId);
      int index = (it == allIds.end()) ? -1 : distance(allIds.begin(), it);
      if (doDebug > 2)
        cout << "\t total ids  " << allIds.size() << " index in SIMHIT vector "
             << index << endl;
      if (index < 0) continue;

      vector<SimHit> tempHits = gemsimHits[index].second;
      if (doDebug > 2)
        cout << "\t GEM simhits found in chamber, size " << tempHits.size()
             << endl;
...
void PadDigiDisplay(GEMDetID id, vector<GEMDetID>& allIds,
                    vector<GEMPAD>& gempads, TH2F* padDis, int doDebug) {
  ...
  for (int i = 1; i < 3; i++) {  // in each interesting layer has strip hits
    for (int ieta = 1; ieta <= 8; ieta++) {
      ...
      int ybin = (8 - tempId.Roll) * 3 + 2 - tempId.Layer;
      label[ybin] << "L" << i << "Eta" << ieta;
      padDis->GetYaxis()->ChangeLabel(ybin + 1, -1, -1, -1, -1, -1,
                                      label[ybin].str().c_str());
      if (i == 1)
        padDis->GetYaxis()->ChangeLabel(ybin + 2, -1, -1, -1, -1, -1, "--");

      if (doDebug > 2)
        cout << "GEMPad display id " << tempId << " label " << label[ybin].str()
             << " ybin " << ybin << endl;
      auto it = find(allIds.begin(), allIds.end(), tempId);
      int index = (it == allIds.end()) ? -1 : distance(allIds.begin(), it);
      if (doDebug > 2)
        cout << "\t GEPads display, total ids " << allIds.size()
             << " index in GEMPads vector " << index << endl;
      if (index < 0) continue;

      vector<GEMPad> tempPads = gempads[index].second;
      if (doDebug > 2)
        cout << "\t GEMpads found in the detector, size " << tempPads.size()
             << endl;
...
void MakeOneLayerGEMPadDisplay(GEMDetID tempId, vector<GEMPad>& pads,
                               TH2F* padDisplay) {
...
void MakeOneLayerGEMSimHitDisplay(GEMDetID tempId, vector<SimHit>& shs,
                                  TH2F* stripDisplay) {
  for (int i = 0; i < int(shs.size()); i++) {
    ...
    int ybin = (8 - tempId.Roll) * 3 + 2 - tempId.Layer + 1;
    // cout<<"Do one GEM layer display strip " << strip <<" ybin "<< ybin <<"
    // pdgId " << pdgid <<endl;
    stripDisplay->SetBinContent(x1 + 1, ybin, pdgid);
  }
}

vector<GEMPadCluster> findClustersInChamber(GEMDetID id,
                                            vector<GEMCLUSTER>& allclusters) {
  vector<GEMPadCluster> clusters;
  for (auto idclusters : allclusters) {
    // cout <<"findCluster in "<< id <<" from allclusters "<< idclusters.first
    // <<" num of cluster "<< idclusters.second.size() << endl;
...
int slopePropagation(int slope, int layer, bool evenChamber, bool ME1a,
                     bool ME1b, string luts_folder) {
  ifstream file;
  string filename;
  string delimiter = " ";
  string line;
  ...
  if (layer == 1) {
    if (evenChamber) {
      if (ME1a) {
        filename = luts_folder +
                   "SlopeCorrection/FacingChambers/"
                   "ExtrapolationBySlope_ME11a_even_GEMlayer1.txt";
      } else if (ME1b) {
        filename = luts_folder +
                   "SlopeCorrection/FacingChambers/"
                   "ExtrapolationBySlope_ME11b_even_GEMlayer1.txt";
      }
    } else {
      if (ME1a) {
        filename = luts_folder +
                   "SlopeCorrection/FacingChambers/"
                   "ExtrapolationBySlope_ME11a_odd_GEMlayer1.txt";
      } else if (ME1b) {
        filename = luts_folder +
                   "SlopeCorrection/FacingChambers/"
                   "ExtrapolationBySlope_ME11b_odd_GEMlayer1.txt";
      }
    }
  } else if (layer == 2) {
    if (evenChamber) {
      if (ME1a) {
        filename = luts_folder +
                   "SlopeCorrection/FacingChambers/"
                   "ExtrapolationBySlope_ME11a_even_GEMlayer2.txt";
      } else if (ME1b) {
        filename = luts_folder +
                   "SlopeCorrection/FacingChambers/"
                   "ExtrapolationBySlope_ME11b_even_GEMlayer2.txt";
      }
    } else {
      if (ME1a) {
        filename = luts_folder +
                   "SlopeCorrection/FacingChambers/"
                   "ExtrapolationBySlope_ME11a_odd_GEMlayer2.txt";
      } else if (ME1b) {
        filename = luts_folder +
                   "SlopeCorrection/FacingChambers/"
                   "ExtrapolationBySlope_ME11b_odd_GEMlayer2.txt";
      }
    }
  }
...
pair<int, int> EighthStripToPad(int eighthStrip, bool evenChamber, bool ME1a,
                                bool ME1b, string luts_folder) {
  ifstream file;
  string filename;
  string delimiter = " ";
  ...
  if (evenChamber) {
    if (ME1a) {
      filename =
          luts_folder + "CoordinateConversion/GEMCSCLUT_pad_es_ME1a_even.txt";
    } else if (ME1b) {
      filename =
          luts_folder + "CoordinateConversion/GEMCSCLUT_pad_es_ME1b_even.txt";
    }
  } else {
    if (ME1a) {
      filename =
          luts_folder + "CoordinateConversion/GEMCSCLUT_pad_es_ME1a_odd.txt";
    } else if (ME1b) {
      filename =
          luts_folder + "CoordinateConversion/GEMCSCLUT_pad_es_ME1b_odd.txt";
    }
  }
    ...
    while (getline(file, line)) {
      ...
      if (eighthStrip == atoi(line.substr(line.find(delimiter), -1).c_str())) {
        ...
        break;
                 ...
                 (eighthStrip - es) *
                         (eighthStrip -
                          atoi(line.substr(line.find(delimiter), -1).c_str())) <
                     0) {
        pad.first = pad_prev;
...
pair<int, int> HalfStripToPad(int halfStrip, bool evenChamber, bool ME1a,
                              bool ME1b, string luts_folder) {
  ifstream file;
  string filename;
  string delimiter = " ";
  ...
  if (evenChamber) {
    if (ME1a) {
      filename =
          luts_folder + "CoordinateConversion/GEMCSCLUT_pad_hs_ME1a_even.txt";
    } else if (ME1b) {
      filename =
          luts_folder + "CoordinateConversion/GEMCSCLUT_pad_hs_ME1b_even.txt";
    }
  } else {
    if (ME1a) {
      filename =
          luts_folder + "CoordinateConversion/GEMCSCLUT_pad_hs_ME1a_odd.txt";
    } else if (ME1b) {
      filename =
          luts_folder + "CoordinateConversion/GEMCSCLUT_pad_hs_ME1b_odd.txt";
    }
  }
...
int WireToRoll(int wireGroup, int layer, bool evenChamber, string luts_folder) {
  ...
  if (evenChamber) {
    if (layer == 1) {
      filename_max =
          luts_folder +
          "CoordinateConversion/GEMCSCLUT_roll_l1_max_wg_ME11_even.txt";
      filename_min =
          luts_folder +
          "CoordinateConversion/GEMCSCLUT_roll_l1_min_wg_ME11_even.txt";
    } else if (layer == 2) {
      filename_max =
          luts_folder +
          "CoordinateConversion/GEMCSCLUT_roll_l2_max_wg_ME11_even.txt";
      filename_min =
          luts_folder +
          "CoordinateConversion/GEMCSCLUT_roll_l2_min_wg_ME11_even.txt";
    }
  } else {
    if (layer == 1) {
      filename_max =
          luts_folder +
          "CoordinateConversion/GEMCSCLUT_roll_l1_max_wg_ME11_odd.txt";
      filename_min =
          luts_folder +
          "CoordinateConversion/GEMCSCLUT_roll_l1_min_wg_ME11_odd.txt";
    } else if (layer == 2) {
      filename_max =
          luts_folder +
          "CoordinateConversion/GEMCSCLUT_roll_l2_max_wg_ME11_odd.txt";
      filename_min =
          luts_folder +
          "CoordinateConversion/GEMCSCLUT_roll_l2_min_wg_ME11_odd.txt";
    }
  }
...
void LCTPropagationDisplay(GEMDetID id, vector<CorrelatedLCT>& lcts,
                           TH2F* extrapLCT, TH2F* unextrapLCT,
                           string luts_folder) {
  ...
  for (CorrelatedLCT lct : lcts) {
    ...
    for (int layer = 1; layer <= 2; layer++) {
      int slope_propagation =
          slopePropagation(slope, layer, evenChamber, ME1a, ME1b,
                           luts_folder);  // Extrapolated slope
      int eighthStrip_extrap =
          eighthStrip -
          slope_propagation *
              ((bend * 2) - 1);  // Extrapolated eighthstrip position

      pair<int, int> x_unextrap =
          EighthStripToPad(eighthStrip, evenChamber, ME1a, ME1b, luts_folder);
      // pair<int, int> x_extrap = EighthStripToPad(eighthStrip_extrap,
      // evenChamber, ME1a, ME1b, luts_folder);

      int roll = WireToRoll(wireGroup, layer, evenChamber,
                            luts_folder);  // Eta partition
      int ybin = (8 - roll) * 3 + 2 - layer + 1;

      int match_window;
      (evenChamber) ? match_window = 20 : match_window = 40;
      // Extrapolated matching window
      pair<int, int> window_high =
          EighthStripToPad(eighthStrip_extrap + match_window, evenChamber, ME1a,
                           ME1b, luts_folder);
      pair<int, int> window_low =
          EighthStripToPad(eighthStrip_extrap - match_window, evenChamber, ME1a,
                           ME1b, luts_folder);
