...
class GifDisplay : public edm::one::EDAnalyzer<edm::one::SharedResources> {
 ...
 private:
  ...
  void fillALCT(edm::Handle<CSCALCTDigiCollection> digicollection,
                vector<CSCIDLCTs>& alct_container);
  void fillCLCT(edm::Handle<CSCCLCTDigiCollection> digicollection,
                vector<CSCIDLCTs>& clct_container);
  void fillCorrLCT(edm::Handle<CSCCorrelatedLCTDigiCollection> digicollection,
                   vector<CSCIDLCTs>& lct_container);
  void fillGEMCluster(GEMPadDigiClusterCollection gemclusters,
...

{
  ...
  // theRootFileName=
  // iConfig.getUntrackedParameter<std::string>("rootFileName");
  ...
  */
  ...
  // comparatorDigiTag =
  // iConfig.getParameter<edm::InputTag>("comparatorDigiTag");
  simHitTagSrc = consumes<edm::PSimHitContainer>(
      iConfig.getUntrackedParameter<edm::InputTag>("simHitTagSrc")),
  gemsimHitTagSrc = consumes<edm::PSimHitContainer>(
      iConfig.getUntrackedParameter<edm::InputTag>("gemsimHitTagSrc")),
  simTrackTagSrc = consumes<edm::SimTrackContainer>(
      iConfig.getUntrackedParameter<edm::InputTag>("simTrackTagSrc")),
  wireDigiTagSrc = consumes<CSCWireDigiCollection>(
      iConfig.getUntrackedParameter<edm::InputTag>("wireDigiTagSrc")),
  stripDigiTagSrc = consumes<CSCStripDigiCollection>(
      iConfig.getUntrackedParameter<edm::InputTag>("stripDigiTagSrc")),
  compDigiTagSrc = consumes<CSCComparatorDigiCollection>(
      iConfig.getUntrackedParameter<edm::InputTag>("compDigiTagSrc")),
  // alctDigiTag = iConfig.getParameter<edm::InputTag>("alctDigiTag");
      alctDigiTagSrc = consumes<CSCALCTDigiCollection>(
          iConfig.getUntrackedParameter<edm::InputTag>("alctDigiTagSrc"));
  clctDigiTagSrc = consumes<CSCCLCTDigiCollection>(
      iConfig.getUntrackedParameter<edm::InputTag>("clctDigiTagSrc"));
  corrlctDigiTagSrc = consumes<CSCCorrelatedLCTDigiCollection>(
      iConfig.getUntrackedParameter<edm::InputTag>("corrlctDigiTagSrc"));
  gemPadTagSrc = consumes<GEMPadDigiCollection>(
      iConfig.getUntrackedParameter<edm::InputTag>("gemPadTagSrc"));
  gemPadClusterTagSrc = consumes<GEMPadDigiClusterCollection>(
      iConfig.getUntrackedParameter<edm::InputTag>("gemPadClusterTagSrc"));

  alctDigiTagSrc_emul = consumes<CSCALCTDigiCollection>(
      iConfig.getUntrackedParameter<edm::InputTag>("alctDigiTagSrc_Emul"));
  clctDigiTagSrc_emul = consumes<CSCCLCTDigiCollection>(
      iConfig.getUntrackedParameter<edm::InputTag>("clctDigiTagSrc_Emul"));
  corrlctDigiTagSrc_emul = consumes<CSCCorrelatedLCTDigiCollection>(
      iConfig.getUntrackedParameter<edm::InputTag>("corrlctDigiTagSrc_Emul"));

  eventDisplayDir = iConfig.getUntrackedParameter<std::string>(
      "eventDisplayDir",
      "/home/mhl/public_html/2017/20171025_cscSeg/eventdisplay/");
  eventlistFile =
      iConfig.getUntrackedParameter<std::string>("eventList", "eventList.txt");
  std::cout << "Eventlist file " << eventlistFile << " outfolder "
            << eventDisplayDir
            << (addEmulation ? " addEmulation" : " NOEmulation") << std::endl;
  ...
  while (file) {
    ...
    if (doDebug > 1)
      cout << "Eventlist: run " << run << " event " << event << " endcap "
           << endcap << " station " << station << " ring " << ring
...
void GifDisplay::analyze(const edm::Event& iEvent,
  ...
  if (addSimHits) {
    ...
    try {
      ...
      if (doDebug > 1)
        cout << "Event " << Event << " collecting simHits " << endl;
      for (const auto& sh : *simHits.product()) {
        ...
        CSCDetId id(sh.detUnitId());  // layer id
        CSCDetID tmpID;
        SimHit tmpSH;
        ...
        tmpSH.Strip = id.ring() == 4 ? layerG->nearestStrip(lp) + 64
                                     : layerG->nearestStrip(lp);
        ...
        // cout <<"chamber "<< id <<" CSC simhit pdgid "<< pdgid <<" wg "<<
        // tmpSH.WireGroup <<" strip "<< tmpSH.Strip << endl;
        bool found = false;
        for (auto& idsh : simhit_container) {
          if (idsh.first == tmpID) {
            idsh.second.emplace_back(tmpSH);
            found = true;
            // cout <<"found exsit id "<< tmpID <<" size of simhits "<<
            // idsh.second.size() << std::endl;
            break;
          }
        }
        if (not found) {
      ...
      if (doDebug > 1)
        cout << "Event " << Event << " collecting gem simHits " << endl;
  ...
  if (addSimHits) {
    ...
    try {
      ...
      if (doDebug > 1)
        cout << "Event " << Event << " collecting simTracks " << endl;
  ...
  if (doDebug > 2 and addSimHits) {
    ...
    //    cout <<"CSC simtcontainer id "<< id_sh.first <<" size "<<
    //    id_sh.second.size() << endl;
    for (auto id_sh : gemsimhit_container)
      cout << "GEM simtcontainer id " << id_sh.first << " size "
           << id_sh.second.size() << endl;
  }
  ...
  if (doDebug > 1)
    cout << "Event " << Event << " collecting wire digi " << endl;
  for (CSCWireDigiCollection::DigiRangeIterator wi = wires->begin();
    ...
    for (; wireIt != lastWire; ++wireIt) {
      ...
      if (doDebug > 2)
        cout << "wg: " << tmpWG.WireGroup << ", time: " << tmpWG.TimeBin
             << endl;
  ...
  // consumes<CSCStripDigiCollection>(stripDigiTag);
  edm::Handle<CSCStripDigiCollection> strips;
  // iEvent.getByLabel(wireDigiTag,wires);
  iEvent.getByToken(stripDigiTagSrc, strips);

  if (doDebug > 1)
    cout << "Event " << Event << " collecting strip digi " << endl;
  for (CSCStripDigiCollection::DigiRangeIterator si = strips->begin();
    ...
    for (; stripIt != lastStrip; ++stripIt) {
      ...
      if (id.ring() == 4)
        tmpSP.Strip += 64;  // for now ME11A only shift 2cfeb, BECAUSE GIF ME11A
                            // ONLY HAS dcfeb 1,5,7
      ...
      if (doDebug > 2) cout << id << endl;
      if (doDebug > 2 && maxADC > 0)
        cout << "strip: " << tmpSP.Strip << ", adc: " << maxADC << endl;
  ...
  // consumes<CSCComparatorDigiCollection>(comparatorDigiTag);
  edm::Handle<CSCComparatorDigiCollection> comparators;
  // iEvent.getByLabel(wireDigiTag,wires);
  iEvent.getByToken(compDigiTagSrc, comparators);

  if (doDebug > 1)
    cout << "Event " << Event << " collecting comparator digi " << endl;
  for (CSCComparatorDigiCollection::DigiRangeIterator com =
           comparators->begin();
       com != comparators->end(); com++) {
    CSCDetId id = (CSCDetId)(*com).first;

    std::vector<CSCComparatorDigi>::const_iterator comparatorIt =
        (*com).second.first;
    std::vector<CSCComparatorDigi>::const_iterator lastComparator =
        (*com).second.second;
    ...
    for (; comparatorIt != lastComparator; ++comparatorIt) {
      ...
      if (id.ring() == 4)
        tmpCOM.Strip += 64;  // gif for now, same reason as strip
      ...
      if (doDebug > 2) cout << id << endl;
      if (doDebug > 2)
        cout << "strip: " << tmpCOM.Strip
             << ", comparator: " << tmpCOM.ComparatorNumber << endl;
  ...
  if (doDebug > 1)
    cout << "Event " << Event << " collecting  GEMPadDigi " << endl;
  ...
  for (auto it = gempads.begin(); it != gempads.end(); ++it) {
    ...
    for (auto pad = padsvec.first; pad != padsvec.second; ++pad) {
      ...
      if (doDebug > 1)
        cout << " gempad: " << tmpPad.Pad << ", bx: " << tmpPad.TimeBin << endl;
  ...
  if (doDebug > 1)
    cout << "Event " << Event << " collecting  ALCT/CLCT/LCT " << endl;
  // ALCT
  ...
  if (doDebug > 1)
    cout << "Event " << Event << " collecting  GEMClusters " << endl;
  ...
  if (addEmulation) {
    if (doDebug > 1)
      cout << "Event " << Event << " collecting Emulated ALCT/CLCT/LCT "
           << endl;
    try {
      ...
      std::cout << " failed to emulated trigger primitives, ignore "
                << std::endl;
      addEmulation = false;
    }
  }
  ...
  // if (Event < 500){//find(eventList.begin(), eventList.end(), Event) !=
  // eventList.end()){ if (find(eventList.begin(), eventList.end(), Event) !=
  // eventList.end()){
  ...
  for (int i = 0; i < int(eventList.size()); i++) {
    ...
    WireStripDisplay(eventDisplayDir, tmpId, simhit_container,
                     simtrack_container, wire_container, strip_container,
                     com_container, alct_container, alct_emul_container,
                     clct_container, clct_emul_container, lct_container,
                     lct_emul_container, usedChamber, Run, Event, addEmulation,
                     compareA, compareB);
    // if (stationL == 1 and (ringL== 1 or ringL==2))
    if (doGEMDisplay and stationL == 1 and (ringL == 1)) {  // only GE11 now
      if (doDebug)
        cout << "doGEMdisplay, station  " << stationL << " endcap " << endcapL
             << " chamber " << chamberL << endl;
      GEMDetID gemId;
      ...
      GEMPadDisplay(eventDisplayDir, gemId, tmpId, gemsimhit_container,
                    gempad_container, gemcluster_container, usedGEMChamber,
                    lct_container, lct_emul_container, Run, Event, addEmulation,
                    doGEMCSC, doDebug);
    }
  }
...
void GifDisplay::fillALCT(edm::Handle<CSCALCTDigiCollection> digicoll,
                          vector<CSCIDLCTs>& alct_container) {
  for (CSCALCTDigiCollection::DigiRangeIterator j = digicoll->begin();
       j != digicoll->end(); j++) {
...
void GifDisplay::fillCLCT(edm::Handle<CSCCLCTDigiCollection> digicoll,
                          vector<CSCIDLCTs>& clct_container) {
  for (CSCCLCTDigiCollection::DigiRangeIterator j = digicoll->begin();
       j != digicoll->end(); j++) {
...
void GifDisplay::fillCorrLCT(
    edm::Handle<CSCCorrelatedLCTDigiCollection> digicoll,
    vector<CSCIDLCTs>& lct_container) {
  for (CSCCorrelatedLCTDigiCollection::DigiRangeIterator j = digicoll->begin();
       j != digicoll->end(); j++) {
    ...
    for (CSCCorrelatedLCTDigiCollection::const_iterator digiIt = range.first;
      ...
      tmplct.BX = digiIt->getBX();
      tmplct.eighthStrip =
          (tmplct.keyStrip * 4 + digiIt->getQuartStripBit() * 2 +
           digiIt->getEighthStripBit());
...
void GifDisplay::fillGEMCluster(GEMPadDigiClusterCollection gemclusters,
                                vector<GEMCLUSTER>& gemcluster_container) {
  for (auto it = gemclusters.begin(); it != gemclusters.end(); ++it) {
    ...
    for (auto cluster = clvec.first; cluster != clvec.second; ++cluster) {
      ...
      if (doDebug > 1) {
        cout << " firstPad " << tmpcluster.firstPad << " size "
             << tmpcluster.size << " BX " << tmpcluster.BX << endl;
      }
    }
...
// ------------
void GifDisplay::beginJob() {}

// ------------ method called once each job just after ending the event loop
// ------------
void GifDisplay::endJob() {}

// ------------ method fills 'descriptions' with the allowed parameters for the
// module  ------------
void GifDisplay::fillDescriptions(
